...
class ApiTestRun(EngineTestSuite):
    def test_server_start_stop(self, test_server):
        ...
        try:
            requests.get(url=url.start, timeout=10)
    ...
    def test_double_start(self, test_server):
        """Проверка двойного старта"""
        _ = self
        url = test_server
        res = requests.get(url=url.start, timeout=10)
        assert res.status_code == 200, 'Engine не был запущен'
        res = requests.get(url=url.start, timeout=10)
        assert res.status_code == 400, f'Engine не вернул ожидаемый status_code (400) а вернул {res.status_code}'
        # проверка что состояние движка - запущен
        res = requests.get(url=url.parameters)
        data = res.json()
        assert data is not None
        running = data.get('parameters', {}).get('running', None)
